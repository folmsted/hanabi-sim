...
class Card:
    ...
    def __lt__(self, other):
        ...
        return self.color.value  * MAX_CARD_VALUE + self.number < \
               other.color.value * MAX_CARD_VALUE + other.number
...
class GameState:
    ...
    def copy(self):
        cpy = GameState()
        cpy.players = [p.copy() for p in self.players]
...
class Player:
    ...
    def perform_discard(self, position, card, game, verbose=False):
        ...
        #Replenishment
        player = new_state.get_player(game.player_up)
    ...
    def perform_play(self, position, card, game, verbose=False):
        ...
        try: new_state.outstanding_cards = new_state.outstanding_cards.remove(card)
            ...
            errstr = f'The card you specified, {card}, is exhausted '\
                     f'by prior plays and discards. (see "show outstanding")' 
            raise HanabiSimException(errstr)
        player = new_state.get_player(game.players.index(self)) #get player in new state
        if (new_state.num_in_deck > 0):
            new_state.num_in_deck -= 1
            player.hand = player.hand.replace_card(new_state.round, position, player)
        else:
    ...
    def perform_hint(self, target_player, positions, hint, game, verbose=False):
        ...
        new_state.hints -= 1
        new_state.previous_state = game
        player = new_state.get_player(game.players.index(target_player))
        try: 
